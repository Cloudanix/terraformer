...
type TimestreamQueryGenerator struct {
	AWSService
}

func (g *TimestreamQueryGenerator) InitResources() error {
	...
	for p := timestreamquery.NewListScheduledQueriesPaginator(svc, &timestreamquery.ListScheduledQueriesInput{}); p.HasMorePages(); {
		...
		for _, q := range page.ScheduledQueries {
			arn := StringValue(q.Arn)
			if arn == "" {
				continue
			}
			g.Resources = append(g.Resources, terraformutils.NewSimpleResource(
				arn, arn, "aws_timestreamquery_scheduled_query", "aws", defaultAllowEmptyValues))
		}
	}
	return nil
}
