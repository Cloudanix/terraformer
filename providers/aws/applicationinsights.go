...
func (g *ApplicationInsightsGenerator) InitResources() error {
	...
	for p.HasMorePages() {
		...
		for _, item := range page.ApplicationInfoList {
			id := StringValue(item.ResourceGroupName)
			if id == "" {
				continue
			}
			g.Resources = append(g.Resources, terraformutils.NewSimpleResource(
				id, StringValue(item.ResourceGroupName), "aws_applicationinsights_application", "aws", defaultAllowEmptyValues))
		}
	}
	return nil
}
