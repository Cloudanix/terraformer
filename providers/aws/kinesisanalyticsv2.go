...
type KinesisAnalyticsV2Generator struct {
	AWSService
}

// InitResources enumerates Kinesis Analytics v2 applications. Import ID is the name.
func (g *KinesisAnalyticsV2Generator) InitResources() error {
	...
	for p.HasMorePages() {
		...
		for _, app := range page.ApplicationSummaries {
			name := StringValue(app.ApplicationName)
			if name == "" {
				continue
			}
